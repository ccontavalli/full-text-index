...
#define NUM_MAX_RULES 3125000 //100.000.000 (*32)

/* define for debuging */
#define ID_CHEQ 3116400000
#define DEBUGME1
...
ulong BPE::compress(double cutoff) {
  ulong i,j,p1=0,p2=0,p3=0,p,f1,f2,a1,a2,aux,aux1=0,aux2=0,aux_value,pp,q1,q2,count,pp1,pp2,auxp,anterior,num,*visit,cuentame=0,estimado=0;
  bool list1,list2;
  ...
  while (p != n-1)  { 
#ifdef DEBUGME
    if (p == ID_CHEQ) {
      if (verbose) printf("Set  %lu %lu \n",p, max_assigned);
    } 
#endif    
    if ((p != 0 ) && ((p%max_phrase) != 0 )) {
      if (bitget(bit_id,p-1) == 0) p1=prev(p-1); else p1 = p-1;
      tp1=frec->search(p1);
      //if ((p1 % max_phrase != 0) && (tp1 !=NULL) && (tp1->value != nodo->value) && (p1 != anterior)) {
      if ((tp1 !=NULL) && (tp1->value != nodo->value) && (p1 != anterior)) {
#ifdef DEBUGME 
       if (p1 == ID_CHEQ) {
          if (verbose) printf("Tipo P1  %lu %lu %lu \n",p1,p, max_assigned);
       } 
#endif
       f1=tp1->priority -1;
       aux_value=tp1->value;
...
#ifdef DEBUGME 
            if (a1+phd[a1] == n-1) {
              if (verbose) printf("P %lu C1 %lu %lu [%lu,%lu] ",p, p1,tp1->value,data[p1],data[tp1->value]);
              if (bitget(bit_id,p1+1) == 0) aux=p1+1+data[p1+1]; else aux = p1+1;
              if (verbose) printf("[%lu,",data[aux]);
              if (bitget(bit_id,tp1->value+1) == 0) aux=tp1->value+1+data[tp1->value+1]; else aux = tp1->value+1;
...
#ifdef DEBUGME        
    if (p2 == ID_CHEQ) {
      if (verbose) printf("Tipo P2 %lu \n",max_assigned);
      
    }
#endif
    if ((p3 % max_phrase != 0) && (p2 != n-1) && (p3 != n-1)) {
      tp2=frec->search(p2);
...
#ifdef DEBUGME
            if (a2+phd[a2] == n-1) {
              if (verbose) printf("P %lu C2 %lu %lu [%lu,%lu] ",p, p2,tp2->value,data[p2],data[tp2->value]);
              if (bitget(bit_id,p2+1) == 0) aux=p2+1+data[p2+1]; else aux = p2+1;
              if (verbose) printf("[%lu,",data[aux]);
              if (bitget(bit_id,tp2->value+1) == 0) aux=tp2->value+1+data[tp2->value+1]; else aux = tp2->value+1;
              if (verbose) printf("%lu]\n",data[aux]);
              exit(0);
            }
#endif
    ...
    if (p2==p+phd[p]) {aux=p2+phd[p2]; phd[p]=aux-p;}
...
#ifdef DEBUGME    
    if (p2 == ID_CHEQ) {
      if (verbose) printf("Set to en  %lu \n",max_assigned);
    }
#endif 
...
#ifdef DEBUGME    
    if (p == ID_CHEQ) {
      if (verbose) printf("Count tipo p2 %lu \n",max_assigned);
    }
#endif 
    if (bitget(visit,i) != 1) {
      if (bitget(bit_id,p+1) == 0) p2=p+1+data[p+1]; else p2 = p+1;
...
/*
      ...
      while (pp != n-1) {
        if (bitget(bit_id,pp+1) == 0) pp2=pp+1+data[pp+1]; else pp2 = pp+1;
        if ((pp2 % max_phrase != 0) && (pp2 != n) && (q1==data[pp2])) {aux=pp-p;break;} 
        pp+=phd[pp];
      }
      p+=phd[p];
      if (aux != n) setphd(auxp,aux); else setphd(auxp,n-1-auxp);
*/
    } else {
      if (bitget(bit_id,p-1) == 0) p1=prev(p-1); else p1 = p-1;
      q1=data[p1];
      ...
      while ((pp != n-1) && (!list1 || !list2)) {
        if ((!list1) && (pp%max_phrase != 0)) {
          if (bitget(bit_id,pp-1) == 0) pp1=prev(pp-1); else pp1 = pp-1;
          if (q1==data[pp1]) {aux1=pp1-p1;list1=true;}
        }
        if (!list2) {
          if (bitget(bit_id,pp+1) == 0) pp2=pp+1+data[pp+1]; else pp2 = pp+1;
          if ((pp2 % max_phrase != 0) && (pp2 != n) && (q2==data[pp2])) {aux2=pp-p;list2=true;} 
    ...
    p+=phd[p];
    if (aux1 != n) setphd(p1,aux1); else setphd(p1,n-1-p1);
  ...
  if ((verbose) && (paraaquinomas> porcentajedeavance1/50.0)) {
  //if (cuentame++ % 10000 == 9999 ) {
    printf("ita%2lu %lu, Frec= %lu, size = %lu, total = %lu, (%.6f%%)", porcentajedeavance1,
...
BPE::BPE(ulong *a, ulong n, ulong _max_phrase, double _cutoff, bool _verbose) {
  verbose=_verbose;
  data=a;
...
BPE::BPE(ulong *sa_diff,ulong *_Psi,ulong _ini,ulong length, bool _verbose){
  /* Global variables */
  verbose=_verbose;
  data=sa_diff;
  Psi=_Psi;
