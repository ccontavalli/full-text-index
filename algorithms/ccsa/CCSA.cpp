...
class TSA {
  ...
  public:
    ...
    TSA(ulong bits, ulong n){
      this->n = n; this->posbits = bits; 
      ulong aux_int = (ulong) (((double) n) * ((double) posbits)/ ((double) W));
      pos = new ulong[aux_int+2];
...
class TCCSA {
  ...
  public:
    ...
    ulong SpaceRequirement() {
      ulong size=0;
      size+= sizeof(TCCSA);
      size+= charrank->SpaceRequirementInBits()/8; //charrank
      size+= posrank->SpaceRequirementInBits()/8; // posrank
      size+= samplerank->SpaceRequirementInBits()/8; // samplerank
      ulong aux_int = (ulong) (((double) (length+1)) * ((double) bitsn)/ ((double) W));
      size+= (aux_int+1)*sizeof(ulong); // pos OK!
      size+= ((Htbits*samplen)/W+1)*sizeof(ulong); //Ht OK!
      size+= size_uchar*sizeof(ulong); // chars OK!
      size+= ((samplen*samplebits)/W+2)*sizeof(ulong); // sample
      return size;
    };
    ...
    ~TCCSA() {
     ...
     int save(char *filename) {
        ...
        ulong aux_int = (ulong) (((double) (length+1)) * ((double) bitsn)/ ((double) W));
        if (fwrite (pos,sizeof(ulong),aux_int+1,f) != aux_int+1) return 21;
     ...
     int load(char *filename) {
        ...
        if (error !=0) return error;
        ulong aux_int = (ulong) (((double) (length+1)) * ((double) bitsn)/ ((double) W));
        this->pos = new ulong[aux_int+1];
        if (!this->pos) return 1;
        if (fread (pos,sizeof(ulong),aux_int+1,f) != aux_int+1) return 25;
