...
int compare_ulong(const ulong *a, const ulong *b)
{
    return *a - *b;
}
...
do_count ()
{
	...
	while (res_patt)
	{
	  ...
	  if (fread (pattern, sizeof (*pattern), length, pfile) != length)
	    {
	  ...
	  if (Verbose) {
	    ...
	    fprintf(stdout,"len=%d pattern=", (uint)length);
	    for (i=0;i<(int)length;i++) fprintf(stdout,"%c",pattern[i]);
	    fprintf(stdout," numocc=%d\n",(uint)numocc);
	  }
...
do_locate ()
{
	...
	while (numpatt) {
		...
		if (Verbose) {
		  ...
		  for (i=0;i<(int)length;i++) fprintf(stdout,"%c",pattern[i]);
		  fprintf(stdout," numocc=%d\n",(uint)numocc);
		  qsort(occ, (int)numocc, sizeof(ulong), (int (*)(const void*, const void*))compare_ulong);
		  for (i=0;i<(int)numocc;i++) fprintf(stdout,"%d\n",(int)occ[i]);
			}
...
void do_display(ulong numc) {
	...
	int error = 0;
	ulong tot_numocc=0,numocc, length, i, *snippet_len, tot_numcharext = 0, numpatt;
	double time, tot_time = 0, count_time=0;
...
pfile_info (ulong * length, ulong * numpatt)
{
	...
	error = fscanf (pfile, "# number=%lu length=%lu file=%s forbidden=", numpatt,
					length, origfilename);
	if (error != 3)
	{
...
do_extract()
{
	...
	ulong num_pos, from, to, numchars, readen, tot_ext = 0, num_pos1;
	double time, tot_time = 0;
	
	error = fscanf(pfile, "# number=%lu length=%lu file=%s\n", &num_pos, &numchars, orig_file);
	if (error != 3)
	{
	...
	while(num_pos) {
		...
		if (Verbose) {
		  ...
		  fprintf(stdout,"[%10d,%10d] ",(int)from,(int)to);
		  for (i=0;i<(int)readen;i++) fprintf(stdout,"%c",text[i]);
		  fprintf(stdout,"\n");
		}
