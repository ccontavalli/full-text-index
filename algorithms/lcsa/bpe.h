...
private:
  ...
  int ini_psi();
  void compress_psi();
  void compress_psi2(double cutoff, ulong s, ulong delta, ulong gamma);
public:
  ...
  BPE(ulong *sa_diff,ulong *Psi,ulong ini,ulong length, ulong _max_phrase, double _cutoff, ulong s, ulong delta, ulong gamma, bool _verbose);
  ~BPE(); //destructor
