 ...
 * index. 
 */
int count(void *index, uchar *pattern, uint32_t length, uint32_t *numocc);
 ...
 * order.
 */
int locate(void *index, uchar *pattern, uint32_t length, uint32_t **occ, 
		   uint32_t *numocc);
 ...
 */
int extract(void *index, uint32_t from, uint32_t to, uchar **snippet,
...
int display(void *index, uchar *pattern, uint32_t length, uint32_t numc, 
			uint32_t *numocc, 	uchar **snippet_text, uint32_t **snippet_len); 


/* 
 * Obtains the length of the text represented by index. 
 */
int get_length(void *index, uint32_t *length);
 ...
 */
int build_index(uchar *text, uint32_t length, char *build_options, void **index);
 ...
 */
int load_index_mem (void ** index, uchar *compress, uint32_t size);
