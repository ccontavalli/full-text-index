...
BitRankFSparse::BitRankFSparse(ulong *bitarray, ulong n){
  ...
  ulong aux_long = (L*sblock->rank(n/L))/W ;
  blockbit=new ulong[aux_long+1];
...
int BitRankFSparse::save(FILE *f) {
  ...
  if (fwrite (&L,sizeof(ulong),1,f) != 1) return 21;
  return 0;
}
...
int BitRankFSparse::load(FILE *f) {
  ...
  if (error !=0) return error;
  if (fread (&L,sizeof(ulong),1,f) != 1) return 25;
  return 0;
}
