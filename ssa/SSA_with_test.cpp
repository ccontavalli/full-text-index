...
public:
    TFMindex(uchar *text, ulong length, ulong samplerate, bool free_text,ulong factor, bool withload, char *fnamext, int *error) { 
       this->n = length+1;
    ...
    int display(uchar *pattern, ulong m, ulong numc, ulong *numocc, uchar **snippet_text, ulong **snippet_lengths, ulong **occ) {
       ...
       if (sp<=ep) {
          ...
          *snippet_text = (uchar *) malloc((*numocc)*(m+2*numc)*sizeof(uchar));
          ...
          ulong **occ2,**snippet_lengths2;
          uchar **snippet_text2;
          occ2 = (ulong **) malloc(sizeof(ulong*));
          snippet_lengths2 = (ulong **) malloc(sizeof(ulong*));
          snippet_text2 = (uchar **) malloc(sizeof(uchar*));
          *occ2 = (ulong *) malloc((*numocc)*sizeof(ulong));
          *snippet_lengths2 = (ulong *) malloc((*numocc)*sizeof(ulong));
          *snippet_text2 = (uchar *) malloc((*numocc)*(m+2*numc)*sizeof(uchar));
          uchar *text_aux2=*snippet_text2;
          { //test
          ulong locate=0;
          ...
          while (i<=ep) {
            ...
            for (dist=numc;dist>0;dist--) {
             ...
             if ((*occ2)[locate] != (*occ)[locate]) {
               printf("         Errorr l=%lu, n=%lu, o=%lu\n", i, (*occ2)[locate], (*occ)[locate]);
               fflush(stdout);
             }

             //assert ((*occ2)[locate]== (*occ)[locate]);
             skip=dist;
             if (dist != 0)
               for (ulong ind=0;ind< numc-dist;ind++)
                 text_aux2[ind]=text_aux2[dist+ind];
             ...
             ulong j = positions[to/samplerate+1];
             if ((to/samplerate+1) == ((n-2)/samplerate+1))
               skip2 = n-2 - to;
             else
               skip2 = samplerate-to%samplerate-1;
             ...
             for (ulong dist=0;dist<skip2+len;dist++) {
               ...
               if (dist>= skip2)
                 if (c == map0)
                   text_aux2[valor_der-dist]='\0';
                 else
                   text_aux2[valor_der-dist]=c;
             }
             for (ulong dist=0;dist<m;dist++) {
               c=pat[dist];
               if (c == map0)
                 text_aux2[numc-skip+dist]='\0';
               else
                 text_aux2[numc-skip+dist]=c;
             }
                
             (*snippet_lengths2)[locate]=len+m+numc-skip;

             if ((*snippet_lengths2)[locate] != (*snippet_lengths)[locate]) {
               printf("         Errorr i=%lu, n=%lu, o=%lu\n", i,(*snippet_lengths2)[locate],(*snippet_lengths)[locate]);
               fflush(stdout);
             }
             //printf("\n::1::");
             text_aux=(*snippet_text)+((m+2*numc)*locate);
             for (ulong kk=0; kk < (*snippet_lengths)[locate];kk++)
                if (text_aux[kk] != text_aux2[kk]) {
                  printf("         Errorr k=%lu, n=%c, o=%c\n", kk,text_aux2[kk], text_aux[kk]);
                  fflush(stdout);
                }

             //printf("::1::\n::2::");
             //for (ulong kk=0; kk < (*snippet_lengths)[locate];kk++)
                //printf("%c",text_aux2[kk]);
             //printf("::2::\n");
...
int build_index(uchar *text, ulong length, char *build_options, void **index){
  ...
  TFMindex *FMindex = new TFMindex(text,length,samplerate,free_text,factor,withload, fnamext, &error);
  (*index) = FMindex;
  if (error != 0) return error;
  return 0;
}
